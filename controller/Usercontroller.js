        ...
        if (social_login == false) {
            if(!emailverify || emailverify.verify==0){
                ...
                const transporter = nodemailer.createTransport({
                  ...
                  const emailTemplateSource = fs.readFileSync('space-mail-template.hbs', 'utf8');
                  const emailTemplate = handlebars.compile(emailTemplateSource);
                  const templateData = {
                      link: `http://18.156.84.152:3007/api/emailverifiaction/${email}/${referalcode}/${password}`
                    };
                  ...
                  res.send({Success:true,Message:"email verification link sent to your email"})
            }else{
                        ...
                        if (result) {
                            ...
                            if (result) {
                                ...
                                if (token) {
                                    ...
                                    const profiletoken = await Profile.findOneAndUpdate({ userId: token.userId }, {
                                        $set: {
                                            Inapptokens: 100 + (profile.Inapptokens)
                                        }
                                    }, { new: true })
                                    console.log(profiletoken);
                                    const find = await Referal_usedcode.findOne({ referalcode: referalcode })
                                    if(find){
                                    const array = find.referedusers
                                    array.push({ userId: result._id })
                                    const usedreferal = await Referal_usedcode.findOneAndUpdate({ referalcode: referalcode }, {
                                        $set: {
                                            "referedusers": array
                                        }
                                    }, { new: true })
            ...
            if (referalcode) {
                const data = await Referal.findOne({ referalcode: referalcode })
                if (data) {
                    console.log(referalcode,"referalcode data....................................................................................");
                    //    const data = await User.findOne({username:username})
                    const Email = await User.findOne({ email: email })
                    if (Email) {
                        res.send({ Success: false, Message: "email already registered" })
                            ...
                            if (result) {
                                ...
                                if (token) {
                                    ...
                                    // if (usedreferal) {
                                        ...
                                        const value = await add.save()
                                        console.log(value,"usedreferal save data............................................................................");
                                        res.send({ Success: true, Message: "User registered with referal code" })
                                    }
                                    // res.send({Success:true,Message:"User registered with referal code"})
                                } else {
                ...
                if(Email){
                    ...
                    const result = await data.save()
                    res.send({Success:true,Message:"User registered without referal code"})
    ...
    try{
        ...
        if(data){
            if(password==data.password){
                ...
                if(token){
                    const refreshtoken = await User.findOneAndUpdate({email:email},{$set:{
                       accesstoken:token
                    }},{new:true})
...
exports.sendtokens=async(req,res)=>{
    try{
        ...
        const af = []
        var receipt = await web3.eth.getTransactionReceipt(hash).then(value => {
        ...
        if (af[0] == null) {
            const data = await Transaction({
                from: address, Type: "refundtoken", hash: hash, status: null, token:token, userId: id
            })
            const result = await data.save()
            ...
            console.log(ha,"ha................................");
            console.log(wei);
            console.log("Wallets", result, address, "\n", req.body, "\n", JSON.stringify(req.body))
            console.log("a..................................................");
            if (ha) {
            ...
            // }
             if (result.toLowerCase() == address.toLowerCase() && wei == token && ac == "0x23959230b02498E8A7b380f8f2C6F545634e1DB9") {  //"0xD8087bDDBA4330CD44a20fFA84b4A1ee80c1a3D3") {
                console.log("i'm here...........................");
                const data = await Transaction({
                    from: result, type: "addtoken", to: ac, hash: hash, status: bt,token:token, userId: id
                })
    ...
    try{
        ...
        if (!profile) {
            const profile = await Profile({
                ...
                image: `http://18.156.84.152:3007/character/${req.file.filename}`,
...
exports.update_profile=async(req,res)=>{
    try{
        ...
        if(data){
            const update = await Profile.findOneAndUpdate({userId:id},{$set:{
                username:username,
                image:`http://18.156.84.152:3007/character/${req.file.filename}`,
                email:email,
                password:password
            ...
            if(update){
                const user = await User.findOneAndUpdate({_id:id},{$set:{
                    email:email,
                    password:password
                }},{new:true})
...
exports.sendreferal=async(req,res)=>{
    ...
    //  const {referalcode,email}=req.body
     ...
     if(referal){
        console.log("here.....");
        const code = await Referal.findOneAndUpdate({userId:id},{$set:{
...
exports.getreward=async(req,res)=>{
    try{
        ...
        const data = await Profile.findOneAndUpdate({userId:id},{$set:{
...
exports.email = async (req, res) => {
    try {
        ...
        // res.render("index")
        const dat = req.params.id
        const data = await User.findOne({$and:[{_id:dat}, {passwordverification:true}]})
        if(data){
        res.render("index")
        }else{
    ...
    try {
        ...
        if (data) {
            ...
            if (password == confirmpass) {
                const userdata = await User.findOneAndUpdate({ _id: id }, { $set: { password: password,passwordverification:false } }, { new: true })
                if(userdata){
...
const resetPasswordMail = async (email, id) => {
    ...
    const emailTemplate = handlebars.compile(emailTemplateSource);
    const templateData = {
        link: `http://18.156.84.152:3007/email/${id}`,
    ...
    let mailDetails = {
        ...
        // html: `http://localhost:8082/email?${emailId}`
        html: renderedTemplate
    };
...
exports.getaccountbalance = async (req, res) => {
    ...
    // const balanceBNB = web3.utils.fromWei(balanceWei.toString(), "ether");
        ...
        if(response!==null){
            const balanceINUSD=Number(balanceBNB)*response.data['binancecoin'].usd
            console.log(balanceINUSD);
            res.send({ balance: balanceINUSD.toFixed(2),SOMBalance:formattedResult })
...
exports.getallimport_tokens=async(req,res)=>{
    ...
    // const tokenPriceAPI = "https://api.coingecko.com/api/v3/simple/price?ids=binancecoin&vs_currencies=usd";
    const response = await fetchDataWithBackoff('https://api.coingecko.com/api/v3/simple/price?ids=binancecoin&vs_currencies=usd');
    console.log(response.data['binancecoin'].usd);
    // console.log(response.binancecoin.usd);
    const balanceINUSD=balanceBnb*response.data['binancecoin'].usd
    const percentagechamgeURI = await fetchDataWithBackoff('https://api.coingecko.com/api/v3/coins/binancecoin');
...
const userAddress = wallet_address;
 ...
 const response = await fetchDataWithBackoff('https://api.coingecko.com/api/v3/simple/price?ids=tether&vs_currencies=usd');
 const usdtValueInUSD = response.data.tether.usd;
 const USDTinusd=USDTBalance*usdtValueInUSD
 const USDTpercentageURI = await fetchDataWithBackoff('https://api.coingecko.com/api/v3/coins/tether');
 ...
 if(USDTBalance){
    ...
    const response = await fetchDataWithBackoff('https://api.coingecko.com/api/v3/simple/price?ids=usd-coin&vs_currencies=usd');
        const conversionRate = response.data['usd-coin'].usd;
        const usdctousd=conversionRate*USDCBalance
        const usdcpercentageuri = await fetchDataWithBackoff('https://api.coingecko.com/api/v3/coins/usd-coin');
    ...
    if(USDCBalance){
        const busdContractAddress = '0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56';//0x8301f2213c0eed49a7e28ae4c3e91722919b8b47
const userAddress = wallet_address;
...
const balance = await busdContract.methods.balanceOf(userAddress).call();
const BUSDBalance = web3.utils.fromWei(balance, "ether");
const response = await fetchDataWithBackoff('https://api.coingecko.com/api/v3/simple/price?ids=binance-usd&vs_currencies=usd');
const conversionRate = response.data['binance-usd'].usd;
const busdtousd= conversionRate*BUSDBalance
const busduripercentagechange = await fetchDataWithBackoff('https://api.coingecko.com/api/v3/coins/binance-usd');
const marketSummary = busduripercentagechange.data.market_data;
const busdpercentChange24h = marketSummary.price_change_percentage_24h;
if(BUSDBalance){
    res.send({Success:true,BNB:balanceBnb,BNBinUSD:balanceINUSD,BNBpercentagechange:percentChange24h,
        USDT:USDTBalance,USDTinUSD:USDTinusd,USDTpercentagechange:usdtpercentChange24h,USDC:USDCBalance,USDCinUSD:usdctousd,
        USDCpercentagechange:usdcpercentChange24h,BUSD:BUSDBalance,BUSDinUSD:busdtousd,BUSDpercentagechange:busdpercentChange24h})  
}else{
    ...
    exports.post_time=async(req,res)=>{
        ...
        const data = await User.findOneAndUpdate({_id:id},{$set:{login_time:login_time}},{new:true})
    ...
    exports.get_referalcode=async(req,res)=>{
        ...
        exports.closeapp_timimg=async(req,res)=>{
            try{
                ...
                console.log(data);
                if(data){
                    const result = await CloseApptiming.findOneAndUpdate({userId:id},{$set:{
                        time:time
                    }},{new:true})
        ...
        exports.redeemtoken=async(req,res)=>{
            try{
                ...
                // if (chain == "BSC") {
                    const web3 = new Web3(new Web3.providers.HttpProvider("https://bsc-dataseed.binance.org/"))// new Web3(new Web3.providers.HttpProvider("https://data-seed-prebsc-1-s1.binance.org:8545/"))
                    ...
                    if (parseInt(formattedResult) < parseInt(count)) {
                        res.send({ Success: false, Message: "Insufficient token in community wallet" })
                    } else {
                        ...
                        // } else {
                            const privateKey = 'fcb623d08cd0ab041339781569e9a95898fad2ffba2d0b6688ff59cdbc727533' //Your Private key environment variable //8692413c6a18e16269c0977dd6618c1243b462408260def3aa5dccc1613ae317
                            ...
                            async function sendErcToken() {
                                ...
                                if (!qw) {
                                    ...
                                    const transaction = []
                                    const qr = await web3.eth.sendSignedTransaction(qw.rawTransaction).then(value => {
                                        console.log(value);
                                ...
                                if (trans) {
                                    ...
                                    const resu = await Transaction({
                                        hash: da, Type: "redeemtoken", from: af.from, status: dat, userId: id, to: address, token:count
                                    })
                                    ...
                                    const tokenupdate = await Profile.findOneAndUpdate({userId:id},{$set:{Inapptokens:value}},{new:true})
        ...
        exports.refreshtoken=async(req,res)=>{
            try{
                ...
                if(data){
                    const refreshtoken = await jwt.sign({ _id: data._id }, "this is my", { expiresIn: '1d' });
                    const tokenupdate = await User.findOneAndUpdate({accesstoken:token},{$set:{
                            ...
                            if (result) {
                                ...
                                if (result) {
                                    ...
                                    if (token) {
                                        ...
                                        // if (usedreferal) {
                                            const updateuser = await User.findOneAndUpdate({email:email},{$set:{
                                                verify:1
                                            }},{new:true})
